          ...
          <div className="grid gap-4 lg:grid-cols-12">
            <div className="rounded-3xl border border-line bg-surface p-6 shadow-card transition-colors duration-200 sm:p-8 lg:col-span-8">
              ...
              <div
                ...
                role="img"
                aria-label="Illustrative chart of monthly revenue rising from $312k in April to $452k in September"
              >
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={financeSeries} margin={{ top: 10, right: 8, left: -14, bottom: 0 }}>
                    <CartesianGrid stroke={isDark ? "#2D3235" : "#E3E8E9"} vertical={false} />
                    <XAxis
                      dataKey="month"
                      tickLine={false}
                      axisLine={false}
                      tick={{ fill: isDark ? "#9CA3AF" : "#838E90", fontSize: 12 }}
                    />
                    <YAxis
                      tickLine={false}
                      axisLine={false}
                    ...
                    />
                    <Tooltip
                      cursor={{ stroke: "#00C5DA", strokeDasharray: "3 3" }}
                      contentStyle={{
                        backgroundColor: isDark ? "#1E2427" : "#FFFFFF",
                        borderColor: isDark ? "#3A3A3A" : "#E2E8F0",
                        borderRadius: 12,
                        color: isDark ? "#FFFFFF" : "#1E2121",
                        fontSize: 12,
                        boxShadow: isDark
            ...
            <ul className="grid gap-4 sm:grid-cols-2 lg:col-span-4 lg:grid-cols-1">
              {financeMetrics.map((m) => (
                <li
                  key={m.label}
                  className="flex flex-col rounded-2xl border border-line bg-surface p-5 transition-colors duration-200"
                >
                  ...
                  <div className="mt-3 h-1.5 rounded-full bg-canvas overflow-hidden ring-1 ring-line">
        ...
        <Reveal className="mt-4">
          <div className="grid gap-px overflow-hidden rounded-3xl border border-line bg-line sm:grid-cols-3">
            {commissionPipeline.map((c, i) => (
              <div key={c.label} className="bg-surface p-6 transition-colors duration-200">
                <p className="text-sm text-ink-muted">Commission · {c.label}</p>
                <div className="mt-2 flex items-baseline gap-3">
                  <span
                    className={`font-display text-3xl font-extrabold ${
                      i === 2 ? "text-brand-ink dark:text-brand" : "text-ink"
                    }`}
                  >
                    {c.count}
                  </span>
